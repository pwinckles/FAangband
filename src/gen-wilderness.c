...
static coord path_start(int sy, int sx, int ty, int tx)
{
	...
	ty +=
		ABS(ty) - ty - ABS(DUNGEON_HGT - 1 - ty) + (DUNGEON_HGT - 1 - ty);
	tx +=
		ABS(tx) - tx - ABS(DUNGEON_WID - 1 - tx) + (DUNGEON_WID - 1 - tx);
...
static void alloc_paths(int stage, int last_stage)
{
	...
	int py = DUNGEON_HGT / 2, px = DUNGEON_WID / 2, pcoord =
		p_ptr->path_coord;
	...
	for (i = 0; i < MAX_PATHS - 1; i++) {
		...
		path_grids =
			project_path(gp, 512, pathend[i].y, pathend[i].x,
						  ...
						  int *feat, int prob)
{
	...
	while (i != (prob - 1)) {
		...
		for (j = 0; j < 100; j++) {
			ty += ddy[terrain];
			tx += ddx[terrain];
			if (!sqinfo_has(cave->info[ty][tx], SQUARE_ICKY))
				break;
		}
...
extern void mtntop_gen(void)
{
	...
	/* Summit */
	for (i = -1; i <= 1; i++) {
		cave->feat[y + i][x] = FEAT_WALL_SOLID;
		cave->feat[y][x + i] = FEAT_WALL_SOLID;
	}

	/* Count the floors */
	for (y1 = y - b; y1 < y + b; y1++)
		for (x1 = x - a; x1 < x + a; x1++)
			if (cave->feat[y1][x1] == FEAT_ROAD)
				floors++;
	...
	for (y1 = y - b; y1 < y + b; y1++)
		for (x1 = x - a; x1 < x + a; x1++) {
			...
			/* Leave rock */
			if (cave->feat[y1][x1] == FEAT_WALL_SOLID)
				continue;

			/* Leave stair */
			if (cave->feat[y1][x1] == FEAT_MORE)
				continue;
			...
			if (cave->feat[y1][x1] == FEAT_ROAD) {
				...
				if (floors == spot) {
					player_place(y1, x1);
					sqinfo_on(cave->info[y1][x1], SQUARE_ICKY);
					continue;
				}
			}
...
extern void river_gen(void)
{
	...
	path = cave->feat[p_ptr->py][p_ptr->px];
	...
	y = p_ptr->py;
	x = p_ptr->px;
	while ((cave->feat[p_ptr->py][p_ptr->px] == FEAT_WATER)
		   || (cave->feat[p_ptr->py][p_ptr->px] == FEAT_PERM_SOLID))
		p_ptr->px++;
	...
	for (i = 0; i < 25; i++) {
		...
		for (y = cy; y < cy + v_ptr->hgt; y++)
			for (x = cx; x < cx + v_ptr->wid; x++)
				if ((cave->feat[y][x] == FEAT_VOID)
					|| (cave->feat[y][x] == FEAT_PERM_SOLID)
					|| (cave->feat[y][x] == FEAT_MORE_SOUTH) ||
