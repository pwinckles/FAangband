...
static ui_event target_set_interactive_aux(int y, int x, int mode)
{
	...
	while (1) {
		...
		/* A trap */
		if (square_visible_trap(cave, y, x)) {
			trap_type *t_ptr = cave_trap(cave, visible_trap_idx(cave, y, x));
		...
		if (boring || !tf_has(f_ptr->flags, TF_FLOOR)) {
			...
			if (p_ptr->wizard) {
				strnfmt(out_val, sizeof(out_val),
						"%s%s%s%s%s%s, %s (%d:%d).", s1, s2, s3, name, s4,
						s5, coords, y, x);
			} else {
				strnfmt(out_val, sizeof(out_val), "%s%s%s%s%s%s, %s.", s1,
						s2, s3, name, s4, s5, coords);
			}
