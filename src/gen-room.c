...
{
	feature_type *feature = &f_info[feat];

	/* Some kinds of terrain are passable. */
	if (feat_ispassable(feature))
		return (TRUE);

	/* Doors are passable. */
	if (tf_has(feature->flags, TF_DOOR_CLOSED))
		return (TRUE);
	...
	if (randint0(36) == 0) {
		...
		/* The ceiling has collapsed. */
			...
			if ((p_ptr->depth > 35) && (randint0(3) == 0)) {
				spread_monsters('X', p_ptr->depth, 2 + randint1(3), y0, x0,
								3, 9);
		...
		/* A lake and/or a forest now fills the room. */
			...
			if (generate_starburst_room
				...
				if ((p_ptr->depth > 15) && (randint0(4) == 0)) {
					spread_monsters('6', p_ptr->depth, 2 + randint1(4), y0,
									x0, 3, 7);
			...
			if ((!water_room) && (randint0(2) == 0)) {
				...
				/* Animals love trees. */
				if (randint0(6) == 0)
					spread_monsters('3', p_ptr->depth, 2 + randint0(6), y0,
									x0, 4, 11);
				else if (randint0(3) == 0)
					spread_monsters('A', p_ptr->depth, 2 + randint0(5), y0,
									x0, 4, 11);
			}
		}
	}
	...
	case 2:
		{
			...
			if (randint0(3) == 0) {
				...
				/* Monsters */
				spread_monsters('\0', p_ptr->depth, randint1(4), y0, x0, 2,
								7);
	...
	case 3:
		{
			...
			if (randint0(3) == 0)
				spread_monsters('N', p_ptr->depth, randint1(6), y0, x0, 2,
								9);
			else if (randint0(3) == 0)
				spread_monsters('*', p_ptr->depth, randint1(6), y0, x0, 2,
								9);
			else
				spread_monsters('\0', p_ptr->depth, randint1(6), y0, x0, 2,
								9);
	...
	case 4:
		{
			...
			spread_monsters('\0', p_ptr->depth, 6 + randint0(11), y0, x0,
							2, 9);
