...
void teleport_away(int m_idx, int dis)
{
	...
	sqinfo_off(cave->info[ny][nx], SQUARE_TEMP);
}
	...
	case 1:
		{
			...
			if ((tmp != m_ptr->r_idx) && (tmp != 0)) {
				...
				delete_monster_idx(cave->m_idx[my][mx]);
	...
	case 3:
		{
			...
			/* Teleport */
			teleport_away(cave->m_idx[my][mx], 8);
	...
	case 12:
		{
			...
			/* Now a tree */
			if (randint1(2) == 1)
				square_set_feat(cave, my, mx, FEAT_TREE);
			else
				square_set_feat(cave, my, mx, FEAT_TREE2);
	...
	case GF_MAKE_DOOR:
		{
			...
			if ((cave->m_idx[y][x] != 0) || (cave->o_idx[y][x] != 0) ||
	...
	case GF_OLD_HEAL:
		{
			...
			if (p_ptr->health_who == cave->m_idx[y][x])
	...
	case GF_NATURE:
		{
			...
			if (!tf_has(f_ptr->flags, TF_ORGANIC)) {
				...
				for (i = 1; i < 10; i++) {
					...
					feature_type *ff_ptr = &f_info[cave->feat[yy][xx]];
	...
	if (who > 0) {
		...
		/* Damaged monster */
			...
			/* Hack -- Pain message */
				...
				message_pain(cave->m_idx[y][x], dam);
...
static bool project_t(int who, int y, int x, int dam, int typ, int flg)
{
	...
	if ((flg & (PROJECT_PLAY)) && (cave->m_idx[y][x] < 0))
		affect_player = TRUE;
	...
	case GF_CHAOS:
		{
			...
			if (dam > randint1(300)) {
				...
				/* Terrain is permuted */
				for (i = 0; i < 7; i++) {
					if (cave->feat[y][x] == terrain_type[i])
						square_set_feat(cave, y, x, terrain_type[randint0(7)]);
				}
			}
			break;
		}
