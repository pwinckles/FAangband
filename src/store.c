...
s32b price_item(object_type * o_ptr, int greed, bool flip)
{
	...
	racial_factor =
		g_info[(ot_ptr->owner_race * z_info->p_max) + p_ptr->prace];
...
static void display_store(void)
{
	...
	/* Normal stores */
		...
		const char *store_name =
			f_info[FEAT_SHOP_HEAD + st_ptr->type].name;
...
static void store_purchase(void)
{
	...
	if (st_ptr->type != STORE_HOME) {
		...
		/* Confirm purchase */
		answer =
			get_char(format("Buy %s for %d gold?", o_name, price), "yn", 2,
					 'y');
...
static void store_sell(void)
{
	...
	if (!get_item
		(&item, q, s, CMD_DROP, (USE_EQUIP | USE_INVEN | USE_FLOOR)))
		return;
	...
	if (st_ptr->type != STORE_HOME) {
		...
		if (!MODE(NO_SELLING)) {
			/* Get the price */
			price =
				price_item(i_ptr, ot_ptr->inflate, TRUE) * i_ptr->number;
			...
			/* Confirm sale */
			answer =
				get_char(format("Accept %d gold?", price), "yn", 2, 'y');
 ...
 */
static void order_item_display(menu_type * menu, int oid, bool cursor,
 ...
 */
static bool order_item_action(menu_type * menu, const ui_event * ev,
							  int oid)
{
...
void store_order(void)
{
	...
	const char cmd_keys[] =
		{ (char) ARROW_LEFT, (char) ARROW_RIGHT, '\0' };
	...
	for (i = 0; i < N_ELEMENTS(order_types); i++)
		if ((i < BOOK_START)
			|| (mp_ptr->spell_book == order_types[i].tval))
			choice[j++] = i;
